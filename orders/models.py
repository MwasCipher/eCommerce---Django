...
class OrderManager(models.Manager):
    ...
    def get_or_create_order(self, billing_profile, cart_object):
        created = False
        qs = self.get_queryset().filter(billing_profile=billing_profile,
                                        cart=cart_object, active=True, status='created')
        if qs.count() == 1:
...
class Order(models.Model):
    ...
    shipping_total = models.DecimalField(decimal_places=2, default=120.00, max_digits=20)
    total = models.DecimalField(decimal_places=2, default=0.00, max_digits=20)
